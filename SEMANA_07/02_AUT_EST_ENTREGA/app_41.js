...
//sorry 
    ...
    app.get('/mostra', function(req, res){
        res.sendFile(__dirname+'/mostra.html')
    ...
    app.get('/mostra', (req, res) => {
        ...
        var sql = 'SELECT * FROM formacao';
            db.all(sql, [],  (err, rows ) => {  
                ...
                console.log('<p>esta funcionando!!</p>')
                res.json(rows);
            });
            db.close(); // Fecha o banco
    });
    ...
    app.post('/inserirdado_page', urlencodedParser, (req, res) => {
        res.statusCode = 200;
        res.setHeader('Access-Control-Allow-Origin', '*'); 
        var db = new sqlite3.Database(DBPATH); // Abre o banco
    ...
    app.put('/mostradado', urlencodedParser, (req, res) => {
